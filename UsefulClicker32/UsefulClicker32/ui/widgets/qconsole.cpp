...
void QConsole::read_stdout()
{
    QByteArray mreaddata =  process->readAll();
    auto str =  QString::fromLocal8Bit(mreaddata);
    //qDebug() << QString::fromLocal8Bit(mreaddata);



/*
    fseek(loggingStreamStdout,0,SEEK_END);
    long size = ftell(loggingStreamStdout);
    fseek(loggingStreamStdout,0,SEEK_SET);
    fread(buffer, 1, size, loggingStreamStdout);
    current_console->append(QString::fromLocal8Bit(buffer));
    //current_console->append(QString::fromLocal8Bit(mreaddata));
*/
    update();
}
...
void PipeReaderThread::run()
{
    ...
    while(1)
    {
        ...
        sleep(1);
        if( pipe.bytesAvailable() )
        //if( pipe.isOpen() )
        {
            QString line = pipe.readAll();
            QConsole* console = static_cast<QConsole*>(parent());
            console->append(line);
            console->update();
            console->append("");
            console->moveCursor(QTextCursor::EndOfLine);
            //qDebug() << line;
        }
    }
}
...
QString QConsole::interpretCommand(const QString &command, int *res)
{
    ...
    // redirect stdout to file stdout.txt to read it after command execution
    //QString modifiedCommand = command + QString(" > \\\\.\\pipe\\StdOutPipe 2>\\\\.\\pipe\\StdErrPipe");// + command + "\"";
    QString modifiedCommand = QString("cmd /C %1 >> %2").arg(command, log_filename);
    ...
    qDebug() << __FUNCTION__ << "executing command : " << modifiedCommand;
